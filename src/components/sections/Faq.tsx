...
export function Faq() {
  return (
    <Section id="faq" className="border-t border-border" innerClassName="stagger max-w-3xl">
      <p className="reveal type-eyebrow text-accent">Questions</p>
      <h2 className="reveal mt-4 type-h2 text-ink">Questions we get asked</h2>

      <div className="stagger mt-10">
        {site.faq.map((item) => (
          <details key={item.q} className="reveal group border-t border-border last:border-b">
            ...
            <p className="faq-answer pb-6 type-body text-ink-muted">{item.a}</p>
          </details>
        ))}
      </div>
    </Section>
