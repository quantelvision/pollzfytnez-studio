...
| gallery | the gallery mosaic, newest upload first | image and video |
...
- `title`: a short headline. Shown over the media in the gallery lightbox.
- `caption`: a sentence under the title, also shown over the media. `description` is accepted as an alias.

Nothing is shown when a field is empty, so an asset with no metadata carries no caption rather than a generated one. Set these per asset in the Cloudinary console under Metadata, Contextual.
...
The gallery reads everything in the gallery folder, images and video together, ordered by upload time with the newest first. Nobody reorders anything: the running order is whatever was uploaded last.

- `GALLERY_PREVIEW_COUNT` sets how many tiles show before the see-all control. Defaults to 10.
- Video tiles are distinguishable without hovering, as the rule always required: a persistent play glyph plus the running time, not a hover reveal. A video's thumbnail is a frame of the video generated by Cloudinary, so no poster files are shipped.
- The mosaic pattern in GalleryMosaic covers exactly twelve cells of the four column grid and repeats, so tiles never leave holes. Dense packing is deliberately not used, because it reorders tiles and the newest-first order has to hold on screen.
- The lightbox is built rather than pulled in, so it takes the theme's colours, radii and motion tokens. It behaves as a dialog: focus moves in and returns to the tile that opened it, Tab is trapped, Escape closes and the arrow keys move through the whole set, not just the preview.
- It carries a thumbnail strip of every item, so any one can be reached in a single click rather than by stepping. The strip scrolls, has its own arrows on wider screens, and keeps the current thumbnail in view.
- The two neighbours either side are fetched ahead of the viewer, so stepping through does not wait on the network. A spinner covers anything not yet decoded, and items already seen never show it again.
- Media is bounded in viewport units rather than by a percentage height. A percentage would need a definite height on every ancestor, and without one a tall image overflowed the screen.
- Mobile: the same strip and controls, plus horizontal swipe to move between items.
- Lightbox video does not autoplay. Browsers block unmuted autoplay anyway, and the reduced-motion rule says nothing plays on its own.
- With an empty or unreadable folder the section shows one labelled placeholder naming the folder it expects, rather than a wall of empty tiles.
